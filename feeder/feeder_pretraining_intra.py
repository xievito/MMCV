...
class Feeder(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if flip_prob < 0.5:
                 data_numpy_v1 = augmentations.joint_corruption(data_numpy_v1_crop)
        else:
        ...
        flip_prob  = random.random()
        if flip_prob < 0.5:
                 data_numpy_v2 = augmentations.joint_corruption(data_numpy_v2_crop)
        else:
...
class Feeder_quadruple(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # temporal crop-resize
        data_numpy_v3_crop = augmentations.temporal_cropresize(data_numpy, number_of_frames, self.l_ratio,
                                                               self.input_size)

        # randomly select  one of the spatial augmentations
        flip_prob = random.random()
        if flip_prob < 0.5:
            data_numpy_v3 = augmentations.joint_corruption(data_numpy_v3_crop)
        else:
            data_numpy_v3 = augmentations.pose_augmentation(data_numpy_v3_crop)

        # temporal crop-resize
        data_numpy_v4_crop = augmentations.temporal_cropresize(data_numpy, number_of_frames, self.l_ratio,
                                                               self.input_size)

        # randomly select  one of the spatial augmentations
        flip_prob = random.random()
        if flip_prob < 0.5:
            data_numpy_v4 = augmentations.joint_corruption(data_numpy_v4_crop)
        else:
        ...
        if self.input_representation == "seq-based" or self.input_representation == "trans-based": 
             ...
             # View 3
             input_v3 = data_numpy_v3.transpose(1, 2, 0, 3)
             input_v3 = input_v3.reshape(-1, 150).astype('float32')

             # View 4
             input_v4 = data_numpy_v4.transpose(1, 2, 0, 3)
             input_v4 = input_v4.reshape(-1, 150).astype('float32')

             return input_v1, input_v2, input_v3, input_v4
