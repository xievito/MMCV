...
parser.add_argument('--warm_up_epoch', default=5, type=int, metavar='N', help='')
...
parser.add_argument('--view', default='joint', type=str,
                    help='...')
...
def load_pretrained(model, pretrained):
    if os.path.isfile(pretrained):
        ...
        # Filter out the keys that start with 'encoder_q'
        filtered_state_dict = {k: v for k, v in state_dict.items() if k.startswith('encoder_q')}
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif args.finetune_dataset== 'pku_v1' and args.protocol == 'cross_view':
        opts = options.opts_pku_v2_cross_view()
    elif args.finetune_dataset== 'pku_v1' and args.protocol == 'cross_subject':
        opts = options.opts_pku_v2_cross_subject()
    ...
    model = moco.mmcv.MoCo(args.finetune_skeleton_representation, opts.bi_gru_model_args, pretrain=False)
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # 将预测结果和真实标签保存到列表中
            result_frag.extend(preds)
            label_frag.extend(target)
    ...
    global conf_matrix
    # 计算混淆矩阵
    conf_matrix = confusion_matrix(label_frag, result_frag)
    # 归一化
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        # np.save('F:/CL/CMD-main/linear_eval_tsne/mmcv/bone/feature.npy', result)
        # np.save('F:/CL/CMD-main/linear_eval_tsne/mmcv/bone/label.npy', label)

        np.save('./conf_matrix/mmcv/result.npy', result)
        np.save('./conf_matrix/mmcv/label.npy', label)
        plt.figure(figsize=(14, 10))
        labels_name = list(range(60))
...
# def adjust_learning_rate(optimizer, epoch, args):
#     """Adjust the learning rate with warm-up"""
#     if epoch < args.warm_up_epoch:
#         lr = args.lr * (epoch + 1) / args.warm_up_epoch
#     else:
#         lr = args.lr * (0.1 ** sum(epoch >= np.array(args.schedule)))
#     for param_group in optimizer.param_groups:
#         param_group['lr'] = lr
