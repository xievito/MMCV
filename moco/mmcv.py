...
class MoCo(nn.Module):
    def __init__(self, skeleton_representation, args_bi_gru, dim=128, K=16384, T=0.07,
        ...
        if not self.pretrain:
            ...
            self.encoder_p = BIGRU(**args_bi_gru)
            self.encoder_q_motion = BIGRU(**args_bi_gru)
            self.encoder_k_motion = BIGRU(**args_bi_gru)
            self.encoder_p_motion = BIGRU(**args_bi_gru)
            self.encoder_q_bone = BIGRU(**args_bi_gru)
            self.encoder_k_bone = BIGRU(**args_bi_gru)
            self.encoder_p_bone = BIGRU(**args_bi_gru)
            weights_init_gru(self.encoder_q)
            weights_init_gru(self.encoder_q_motion)
            weights_init_gru(self.encoder_q_bone)
            ...
            weights_init_gru(self.encoder_p)
            weights_init_gru(self.encoder_p_motion)
            weights_init_gru(self.encoder_p_bone)
    ...
    def reshape_im(self, im):
        ...
        # Permute and Reshape
        N, C, T, V, M = im.size()
        im = im.permute(0, 2, 3, 1, 4).reshape(N, T, -1)
        im_motion = im_q_motion.permute(0, 2, 3, 1, 4).reshape(N, T, -1)
        im_bone = im_q_bone.permute(0, 2, 3, 1, 4).reshape(N, T, -1)
        return im, im_motion, im_bone
